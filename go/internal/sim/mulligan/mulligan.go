...
func (s *Sweep) Gain() float64 {
	return sim.RoundTo(s.Best.SpellsThroughT8-s.Baseline.SpellsThroughT8, 2)
}
...
func row(rule tier1.KeepRule, library []*sim.Card, commander *sim.Card,
	...
	return Row{
		...
		SpellsThroughT8:     sim.RoundTo(summary.SpellsThrough(Through), 2),
		MulliganRate:        sim.RoundTo(summary.MulliganRate, 4),
		AvgMulligans:        sim.RoundTo(summary.AvgMulligans, 3),
		MedianCommanderTurn: summary.MedianCommanderTurn,
		ColorScrewRate:      sim.RoundTo(summary.ColorScrewRate, 3),
		StalledTurns:        sim.RoundTo(summary.AvgStalledTurns, 3),
