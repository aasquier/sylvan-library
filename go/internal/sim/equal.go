...
//
// Tier 1 takes cards out of a hand by removing the first element **equal**
// to its argument, never the identical object it was handed -- the recorded
// remove semantics. A compiled deck repeats one card for a basic's `qty`
// (`sim/compile` repeats the same compiled card qty times), so a hand can
// hold several cards
...
// matters: it distinguishes a nil slice from an empty one, and the recorded
// equality does not -- an absent pip list and an empty one are the same
// answer.
//
// The colour sets are compared as *sets* because a colour set has no order,
// only membership. Every producer of these values -- `sim/compile` through
// the recorded corpora, and the parser through `mana.Cost` -- writes them
// sorted, so a sorted comparison is a set comparison. A caller that builds
// one by hand
// keeps the invariant or gets an answer about a card nobody has.

// Equal compares two cards, field for field.
...
// Equal compares costs: the pip *sequences* in order, each pip's colours as
...
// Equal compares sources: the same colours, and the same amount.
func (s Source) Equal(o Source) bool {
	return s.Amount == o.Amount && SameColors(s.Colors, o.Colors)
}

// SameColors reports whether two colour sets hold the same colours. Both are
// expected sorted and deduplicated, which is how every
...
// Intersects reports whether a mana unit can pay a coloured pip: a non-empty
// intersection. It is the innermost test of the castability matching,
