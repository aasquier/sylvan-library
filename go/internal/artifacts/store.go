...
// Store is `generate.store`: write a rendered build into a directory, and
// prune what it did not make.
//
...
// `swaps.md`.
//
// Shared by every writer of a build for the reason the Python one is: the CLI
...
func Store(files Files, dir string) ([]string, error) {
	// 0755 and 0644 are what Python's `mkdir` and `write_text` leave under the
	// 022 umask the container runs, and `library.FileSource.Create` writes a
	// `deck.yaml` the same way for the same reason: both runtimes write this
	// directory as the same user, nothing else reads it, and a library whose
	// files wear two sets of permissions depending on which door built them is
	// a difference somebody has to explain to a volume backup.
	...
	for _, file := range files {
		...
		if err := os.WriteFile(path, []byte(file.Text), 0o644); err != nil { //nolint:gosec // matches Python's umask
			return nil, err
		}
	...
	for _, name := range Deliverables {
		...
		// `unlink(missing_ok=True)`: a deliverable this build did not produce
		// and the last one did not either is simply absent, which is the
		// ordinary state of `swaps.md` on a first build.
