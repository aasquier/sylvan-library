...
// page's payload is read by people in DevTools as much as by the client — so
// the hand-built bodies in the API are ordered pairs rather than maps.
// encoding/json sorts a map's keys where a Python dict keeps insertion order,
// and that difference has shipped once already: the deck page's Notes tab was
// alphabetical from v159 to v166, scrambling a deliberate reading order into
// nonsense.
//
// These lived in `internal/api` until Phase 6, when the deck reads were
...
// so the HTML-escaping and float rules that make this file agree with
// `json.dumps` apply to every value inside an ordered body too.
