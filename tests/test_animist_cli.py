"""`mtglab animist`, end to end through `main()`, with the network faked
...
def scene(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mtglab.animist.sources._urllib_get", fake_get)
    monkeypatch.setattr("mtglab.animist.fetch._download", fake_download)
    with config.use_paths(data_dir=tmp_path / "data"):
        yield recipe


def test_build_writes_and_says_so(scene, capsys):
    main(["animist", "build", str(scene)])
...
def test_build_dry_run_writes_nothing(scene, capsys):
    main(["animist", "build", str(scene), "--dry-run"])
    out = capsys.readouterr().out
...
def test_build_refused_licence_exits_with_the_gate_sentence(
    ...
    monkeypatch.setattr("mtglab.animist.sources._urllib_get", by_get)
...
def test_verify_with_nothing_to_check_refuses(monkeypatch):
    monkeypatch.setattr("mtglab.cli._animist_all_recipes", list)
    with pytest.raises(SystemExit) as excinfo:
        main(["animist", "verify"])
...
def test_measure_prints_the_curve_and_the_knee(scene, capsys):
    main(["animist", "measure", str(scene), "--output", "canopy.webp",
          "--widths", "16:48:16"])
    out = capsys.readouterr().out
...
def test_measure_needs_a_real_file_output(scene):
    with pytest.raises(SystemExit) as excinfo:
        main(["animist", "measure", str(scene), "--output", "nope.webp"])
    assert "no `file:` output" in str(excinfo.value)


def test_build_unknown_output_is_refused(scene):
    with pytest.raises(SystemExit) as excinfo:
        main(["animist", "build", str(scene), "--output", "nope.webp"])
