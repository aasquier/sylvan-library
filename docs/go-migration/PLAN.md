...
available. **`/api/sim/forge` stayed Python's until Phase 7** (2026-08-23), and while it
did it was what kept the proxy branch live. It is the door's now, so no
Python route creates a job at all — which changes what that branch is *for*
rather than whether it is needed: what is still on the other side is **the
past**, an id minted before the deploy that flipped the last family and
still being polled. Every merge deploys (ADR 23) and a Forge match runs for
minutes, so a match submitted thirty seconds before a release is exactly the
row it exists to keep answerable. **Phase 5 is complete.** **The GIL dividend is banked here, and cannot
...
**Phase 7 — Forge and the ledger** *(~¼–½ day; re-priced ~2–3
hours, and the hosted match is most of it; **done 2026-08-23**).* `sim/tier3` wire
...
snapshot labels at match time, record from exactly two places), coverage
checked twice, medians-and-tails reporting. **`scanruns` is struck from this
list**: ADR 34's camera crossed with Phase 6 (#263), where the rest of
`claude/` was, rather than here beside Forge -- the grouping in this
paragraph was written before the Claude family had a phase of its own. The shim itself re-ships as a Go binary on the
worker (its idle self-stop behavior preserved) — **not a static one, which is
the phase's one correction to its own plan**: `mtglab forge-shim` is a
subcommand of the binary that also serves the front door, and the door links
DuckDB, so `CGO_ENABLED=0 go build ./cmd/mtglab` fails at `internal/pool`.
Two binaries would have bought a static shim at the cost of two artefacts to
keep in step across a deploy that updates the app and the worker minutes
apart — and surviving exactly that gap is what `wire.go` is written for. One
...
- **The port board is the table below**: per module — `python | porting |
  ...
  | `DELETE /api/admin/users/{username}` | **go** | `go/internal/api/admin.go:deleteAccount`, 2026-08-23 (Phase 8's second batch). The blocker this row recorded dissolved with Phase 7: no Python route creates a job any more, so uvicorn's registry is always empty and the door's `jobs.ForgetOwner` count is the honest `jobs_dropped`. The two guards cross intact — `confirm` casefold-equals the username (422, repr-quoted) and an admin cannot delete the account their session belongs to (409) — and `refuseAdminWrite` maps `ErrLastAdmin` exactly as the eleven registrations before it |
  | `/api/admin/stats/*` (the six), `api/traffic.py`, `api/flymetrics.py`, `claude/prices.py` | **go** | `go/internal/api/adminstats.go` + `internal/traffic`, `internal/flymetrics`, `internal/prices`, 2026-08-23 (Phase 8's second batch, and with it **nothing canonical under `/api` is Python's**). The two couplings this row recorded dissolved the same way: the census is the door's own registry (the only one holding jobs), and `_rss` honestly reports the process on the port. Three findings worth the row. **The visitor ledger had been quietly starving** — Python's middleware counts what Python answers, and every flipped family took its requests out of the count; the door now records what the door answers (matched routes by their TEMPLATE, mounts by prefix, the shell as `/{full_path}` — read off the deployed ledger, not guessed — refusals as `(unrouted)`), and a proxied request lands in exactly one ledger during what remains of coexistence. **`estimate`'s dollars needed three of the five reproductions at once** — a running `+=` (never `fsum`: the accumulation order is Python's), `pyfloat.Rounded` on every product (the FMA shape), and `RoundTo(x, 4)` half-to-even — held by a corpus that crosses Sonnet 5's rate changeover in both directions. And **an absent app.db answers Python's empty shapes, never a 404** — Python would mint the database and answer zeroes; the door answers the same zeroes without minting, which is the one divergence kept and it is the safe direction |
  ...
  | `GET /api/forge`, `POST /api/sim/forge` — Tier 3 | **go** | `go/internal/api/forge.go`, 2026-08-23 — **Phase 7, and with it the last job-submitting family.** The gate answers on configuration alone (no network, no machine woken: `/api/claude`'s contract, and a gate that started a JVM would be a bill on every visit to the Simulator). The match keeps the `themeruns` division exactly: a missing deck is 404, an absent Forge 503, a deck Forge cannot fully play a **422 that names the cards**, and only the JVM run is queued. Behind it, `go/internal/sim/tier3` is the whole bridge — the `.dck` exporter, the coverage index over Forge's own 34,532 card scripts, the log parser, the subprocess runner, the wire, the Machines API client — and `go/internal/sim/tier3/ledger` is ADR 36. Three findings worth the row. **`round(ms/1000, 1)` is half-to-even and `encoding/json` writes `4` where Python writes `4.0`**, so every float on this payload goes through `pyfloat.Float`; `ReprFloat` left `internal/sim/tier1` to become `pyfloat.Repr` for it, joining `pyrand`, `pyyaml`, `pyfloat` and `pycasefold` — and `internal/pytext` is a fifth reproduction, for `str.strip()` and `str.splitlines()`, which the Forge log needs because a form feed in a deck name splits a winner line in Python and leaves it whole in Go. **The seed is a `*big.Int`**, because it is echoed into the result, the dedupe key and Forge's own command line, and an int64 would answer a different number than the request named. And **a deck played against itself reports the combined wins on both lines** — `wins` is keyed on the slug — which is reproduced and pinned rather than fixed, the guard beating the fix for a wart nothing in the app can reach. The corpus is 18 log shapes, 7 `.dck` exports, 7 coverage readings and 8 shaped matches; twenty deliberate mutations die against it, and the two that survived the first run bought a corpus row each (a `A // B` name whose *back* face is the only one Forge knows, and a failure that happens **inside** the ledger's transaction rather than before it) |
  ...
  | `claude/argue.py` — the slot argument (ADR 25) | **go** | `go/internal/claude/argue.go` + `go/internal/api/argue.go`, 2026-08-22 — **the mode and its plain route, flipped together**; the SWEEP (`POST .../argue/deck`) is a job and stays Python's, because a prefix is not a family. The route is deliberately the interview's twin down to the status codes — it shares `refuseClaude` rather than growing a second mapping, which means it inherits the stance wart as one wart in one place rather than two spellings of it. Two Python-owned halves carry the design and both are held to a corpus (21 charge cases, 19 alternative cases, rendered against the real 21-card pool). **`OnlyCharges` moves the predicate**: the interview asks "does it end in a question mark", because the failure there is a declarative sentence; every item here is declarative by design, so the predicate becomes "does it rest on anything" and a charge with no `fact` is not a charge. The two enum FALLBACKS are the half a port gets wrong — an unrecognised `ground` or `strength` falls back rather than dropping the charge, because a labelling miss is not a reason to throw away a cited argument, and a test that only counted the good ones would never notice. **`ResolveAlternatives` is rule 2 made executable**, and the corpus uses CLAUDE.md's own first recorded error as its fixture: `tiny_pool` carries *Ajani, Nacatl Pariah // Ajani, Nacatl Avenger*, whose front face is white and whose identity is R/W, so the off-colour case is the real one — asked by front face (how a model names it, and the spelling an index keyed only on the pool's `A // B` name misses SILENTLY) and by full name. The ORDER of the verdicts is part of the contract: already-in-deck is checked first because it is the most specific true thing to say, and Goreclaw's Primeval Titan is both banned and in that deck. **One Python inconsistency was found and reproduced**: `_report`'s default for `alternatives_dropped` lists FOUR keys while `resolve_alternatives` returns FIVE, so a stance-off report and a real run carry different key sets. A single Go struct would have smoothed that into five everywhere — a wire divergence on exactly the paths where no call was made — so there are two types and a test pinning both shapes. 18 mutations, 0 survivors; five survived the first round and every one was a real gap the corpus could not reach (the input dedupe, which the pool-name dedupe downstream masks for any name that RESOLVES; the five-charge cap; the command zone in the in-deck set; and the route's source and tier). Proved on the real wire by a live case of its own |
  | `claude/dossier.py` + `claude/research.py` — the shared source check | **go (engine)** | `go/internal/claude/sources.go`, 2026-08-22 — **the two hosted-search modes' shared instruments; NOTHING FLIPPED.** `CanonicalURL` and `KeepSources` are the rule both modes rest on, and it exists because of a property of the wire rather than a suspicion about the model: a response schema suppresses the API's own citations, so a URL in a payload has nothing behind it but the model's word. Past that the two diverge twice, both ADR 26, and the corpus hands the SAME input to both so neither can be tidied into the other: a dossier passage keeps its prose when every citation failed (it may rest on its brief) where a research finding is dropped and counted (it has no brief, so uncited is resting on recall); and an unresolved rival is a card the model invented and dropped, where an unresolved research card may be one spoiled since the last refresh and is LABELLED `in_pool: false`. Three payload key orders, all Python's, none tidy: `oracle_text` is fifth in a research card and LAST in a competitor, and an unresolved card carries exactly TWO keys — so the three cannot share a type. **A Python bug was found and reproduced**: `_competitors` indexed only the pool's spelling where `resolve_cards` and `argue.resolve_alternatives` index `asked_as` too, so a double-faced card named by its FRONT FACE resolved in research and was dropped as invented by the dossier — measured against the real pool, and commanders are exactly the population most likely to be double-faced. Reproduced rather than fixed, pinned by a test that says so, raised with Aaron — **and ruled and fixed in both runtimes the next day (2026-08-23)**, the corpus regenerated to show the front face resolving in both. **And a systematic port bug of our own**: every instrument in this family spells its field reads `str(...)`, which renders an int as "3" and a bool as "True", where Go's `v.(string)` answers "" — turning a cited finding into a dropped one. `pyStr` fixes it, `Turn.Parsed` now decodes with `UseNumber` so `3` and `3.0` stay distinguishable (the stance parser's fix, one lane later), and the family's THREE spellings are copied per call site rather than harmonised: `str(get(k, ""))`, `str(get(k) or "")` and `str(v or "")` differ on exactly one input each. 19 mutations, 0 survivors — and the passes earned it: one exposed a corpus where every `only_grounded` row was degenerate because the items carried `prose` and not `claim`, and another showed the test decoding corpus numbers as `float64` where production decodes them as `json.Number`, so the branch that actually runs was untested |
  | `claude/dossier.py` + `api/dossierruns.py`, `claude/research.py` + `api/researchruns.py` — the two searching modes, and their three routes | **go** | `go/internal/claude/{dossier,research,pyjson,pytext}.go`, `go/internal/api/{dossier,research}.go`, `go/internal/deckread/commander.go`, 2026-08-23 — **the first Claude JOB families the door answers**: `GET .../dossier` (free, reads the store, never calls), `POST .../dossier` (a job on the NET lane, keyed on the cache key so a second click joins the first — the 2026-08-13 double spend, closed) and `POST /api/claude/research` (a job keyed on the normalised question; no owner, no deck, and a test holds the plan's types free of one — ADR 26's absence is the contract). Both land in the registry the hybrid poll routes already read. **The dossier cache key is Python's byte for byte, unlike the sim cache's, on purpose**: a dossier is prose after a source check both runtimes hold to one corpus, so the rows are shared and nothing already written is paid for again — `Fingerprint` is the same version, the same instruction bytes (modes.json), the same `json.dumps(schema, sort_keys=True)` (pyjson.go, a second transcription of `json.encoder` with the input set closed) and the same model id; the corpus holds the keys and the fingerprint's parts apart. Held to Python by two corpora written with the clock frozen (`dossier.json`, `research.json`): every outcome of a run compared as marshalled bytes (key order is the wire), the brief's whole opening message as bytes, the GET's three shapes (a deck with no commander has five keys and no `answered_by`), `check_question` over every shape a body can carry. Two Go-only divergences fixed in passing: `Converse` prefixed every API failure with the mode name (`apiFailure`: `Error()` is the bare explanation, `Unwrap()` the SDK's error — so a route's 502 and a job's error read as Python's), and the turn ceiling read `mode exhausted: …` without Python's full stop. Two warts reproduced (the GET reads under the default tier's key; a stored row or a stance of off answers before the key is asked for) and one gap recorded (the research in-flight key casefolds in Python and lowercases here; it never leaves the process). Go's GET never creates app.db where Python's would. Three route refusals land before any job — no commander (422), a stance that will not read (422, and the interview's 502 wart was ruled 422 in both runtimes the same day), no key (503) — and a failed call's job error is `explain()`'s sentence, bare. Proved on the real wire by two opt-in live cases and on the instance by a research question driven through the door |
  ...
  | `claude/theme.py` + `api/themeruns.py` — the theme interview, both halves and both routes | **go** | `go/internal/claude/theme.go` and `go/internal/api/theme.go`, 2026-08-23 — **the create flow's first door, and the biggest single module in `claude/` at 1,684 lines.** Both halves are NET jobs and both take **no key**, which is the opposite call from research's dedupe one row up and the reason it is stated rather than inherited: two turns in flight are two *conversations*, because the transcript is client-held and a second tab is a second person's evening. The four instruments ADR 20 rests on all cross, and none of them is the prompt — `Ground` intersects a claimed preference with the text of the person supposedly holding it (user turns only, joined by a separator no keyboard produces so a quote cannot span two of them), `Carry` is the readiness floor that **may not go backwards**, `MayPropose` counts in Go with no `ready` field for a model to set, and every commander is resolved through the pool with an identity that is *exactly* the combination's. A **409** appears here that no other Claude route has: a floor not yet reached is not a malformed request. **Three CPython behaviours had to cross with it, and the first is the reason the lane is not a transliteration.** `Ground` folds the person's own turns and the model's claimed quote and asks whether one contains the other — and `strings.ToLower` is **not** `str.casefold()`. Full case folding turns `ß` into `ss`, `ſ` into `s`, `ς` into `σ`: 211 code points disagree with `lower()` and 104 of them fold to more than one character, so no rune-to-rune mapping can express it. A German answer quoted back at a fortune-teller's table would ground in Python and drop here, and a dropped slot is a readiness count that will not move — commandment 2's failure exactly. So `pyCasefold` joins `pyrand`, `pyyaml` and `pyfloat` as the fourth reproduction of somebody else's arithmetic, over a table swept out of a real interpreter: **the whole of Unicode rather than the 211 differences**, because recording only the disagreements would leave the rest resting on Go's `unicode` tables agreeing with CPython's, which is a claim about two projects' Unicode versions and not one this port gets to make. The neighbouring modes still spell it `ToLower` (argue's in-deck set, interview's card match, research's in-flight key); those fold a pool name against the pool's own spelling of it, where both sides are the same string and the two functions cannot disagree — switching them is a behaviour change to already-flipped routes and wants its own corpus, not a ride-along. Second: the seed goes through **`int()`**, not the Pydantic grammar `/api/tarot/reading` uses — the tarot lane measured those apart, and this one reads the fullwidth `７` that route refuses. `int()` takes any Unicode decimal digit, and Go's `unicode.IsDigit` is category Nd exactly while nothing in the standard library gives a digit's *value*; the obvious walk-down (step back while the neighbour is still a digit) is **wrong for 36 code points**, because the mathematical digit blocks from U+1D7CE are four runs of ten with no gap, so a bold `4` reads as fourteen. Measured before it shipped; the fix is a second swept table of the 68 run starts -- and **the two swept tables then failed CI on 3.11**, which was the right alarm and, on measurement, two different cases: `casefold` is *identical* under 14.0 and 15.0 (1,530 folds, same code points, same strings) and only the `unicode_version` field it recorded had varied, so the field went; the digit runs really do differ (3.12 knows the Kawi and Nag Mundari blocks 3.11 does not), so that table is committed as **3.12's**, the interpreter the container runs, and its freshness check asserts equality on a matching Unicode and a strict *subset* on an older one rather than skipping -- a skip being a finding filed as an absence. The rule that falls out is general: **a sweep over a character property needs the same "which interpreter is the truth" answer a float sum does.** Third: the budget is `f"{v:g}"`, which is Go's `'g'` at precision **6** and not its default −1 — `1234567.0` is `1.23457e+06` in Python and `1.234567e+06` from the default. And `float(budget)` sits inside a `try` that catches `ValueError` and **not** `TypeError`, so a string that will not read is a 422 and a list budget is an uncaught 500 — reproduced rather than tidied, because a flip that changes behaviour is not a flip. The corpus is `testdata/theme.json`: every reader over every shape it refuses, all three prompts as **bytes** (a frame that quietly lost its spread changes what the model was told and shows up in no report), and every outcome of both halves as marshalled bytes — which is what pins the **two report shapes each half has**, since a turn that reached the model carries `never` and one that did not does not, and a proposal that resolved something carries five keys the other four leave off, in the *middle* rather than at the end. 33 deliberate mutations were driven before it was trusted |
  | `claude/theme.py`'s budget parse — `theme.read_budget` and `BudgetRejected` | **both, ruled** | `src/mtglab/claude/theme.py` + `go/internal/claude/theme.go`, 2026-08-23 — **not a port row; a wart the port raised and Aaron ruled.** `float(budget)` sat inside a `try` catching `ValueError` and not `TypeError`, so a *list* budget was an uncaught 500 while an unreadable *string* was a 422 — two spellings of one malformed field, answered two ways, one of them as though the server had broken. And the 422 read `could not convert string to float: 'about fifty quid'`, naming a language builtin in a string the create flow renders verbatim (commandment 10, on the screen a newcomer meets first). Fixed in both runtimes at once, the way `edit.set_shared` and the stance wart went: `BudgetRejected` is a `ValueError`, so the route's existing 422 branch answers and neither runtime needed a new `except`. **The grammar is deliberately unchanged** — the ruling was about what a refusal says, never about what is accepted, so `1_000`, the fullwidth digits, `Infinity` and `1e400` still read exactly as before, checked by reaching the *next* refusal rather than this one. The corpus now drives `read_budget` instead of calling `float()` by hand, and `error_kind` is gone: the distinction it recorded no longer exists, and that absence is the fix |
  | `GET /api/claude` — the dial | **go** | `go/internal/claude/dial.go`, `go/internal/api/claude.go`, 2026-08-23 — **the view flips last, again.** It reports which modes are built and what each surface defaults to, so it could only answer honestly once the modes and their stance owners had crossed. It lives in `internal/claude` rather than `internal/deckread`, where the other read payloads went, because it *cannot*: `claude` already imports `deckread` for the theme brief, so a builder there would be a cycle; the deck reaches it as a status string through the same narrow `DeckStatused` the stance takes. Held to Python by 20 resolutions in `stance.json`, **compared as marshalled bytes** — which caught the `never` sentence carrying a typographic U+2019 where `service.py` writes an ASCII apostrophe (the frontend's own test fixtures spell it the curly way, which is where the guess came from); every field-by-field assertion reads those two as *the never sentence* and passes. Two request orderings are contract and **neither is in the source** — both measured against the running app, and both the opposite of the natural Go shape: the owner is resolved even with **no slug** (Python passes `lib.source_for(...)` as an *argument*, so `?owner=nobody` alone is a 404 rather than a dial), and the deck is read **before** the stance is parsed (both-wrong is the deck's 404). One field has no Python analogue and is argued rather than assumed: `installed` is a constant `true`, since the SDK is linked into the binary rather than riding the `claude` extra — the two agree everywhere the door runs, because the container installs `.[api,claude]`. **Two warts found, reproduced and pinned, and they are one omission wearing two hats**: `?surface=scan` resolves `off` even though `scan.stance_for` exists with a docstring saying in as many words that it is public to prevent exactly that (`_SURFACE_DEFAULTS` names `theme` and `research` only), and the `modes` list is **six of the seven**, last extended by #93 while ADR 34's scan landed in #180 — a payload whose own comment calls itself *the modes that exist*. Both are unreachable from the app today, which is how they survived: nothing sends `surface=scan` and nothing renders `modes`. The Go list is written out rather than derived **precisely because the correct derivation gives seven**, and a test pins the absence against `ModeNames()` so adding a mode fails and asks the question. Four mutations driven; the fifth, hoisting the stance parse, is an equivalent mutant and says so |
  | `claude/scan.py` + `api/scanruns.py` — the camera's fallback tier (ADR 34) | **go** | `go/internal/claude/scan.go`, `go/internal/api/scan.go`, 2026-08-23 — the seventh mode and the smallest, and **the one route in the app that receives a photograph**. A job from its first commit on the NET lane, keyed on the picture so two presses on one shot are one paid call; its duration is still **unmeasured**, which is the reason it is a job rather than an argument that it needs to be one. **What comes back is not a card**, and the port makes that structural rather than stated: `identifyAgainst` was lifted out of the camera route's own handler so the job and the route call **one** function — Python's shape (`scanruns` calls `service.identify_cards`, the route's own), and *the same scrutiny as a card read by Tesseract* has to mean the same code rather than code that looks like it. **The finding is that Go's `base64.StdEncoding.DecodeString` is not `b64decode(validate=True)`**: Go's decoder skips `\r` and `\n` wherever they appear, so the first version accepted two captures Python refuses — one a plain trailing newline, which is exactly what a client that wrapped its base64 at 76 columns sends. The strictness is load-bearing rather than fussy, since the *default* `b64decode` discards non-alphabet characters and a capture with a stray newline would decode shorter and reach the API as a corrupt image; the alphabet and the padding are checked explicitly now. Non-canonical trailing bits stay accepted because Python accepts them — `YW==` decodes to `a` and re-encodes to `YQ==`, which makes the round trip a **normalisation** and is why the dedupe key hashes the re-encoded capture. The sighting is a struct and not a `map[string]string`: Python loops `("title", "corner")` and only adds a key whose stripped value is non-empty, so a map would alphabetise it *and* render an unread field as present-and-blank. One wart reproduced: a capture that is neither a string nor bytes reaches `len()` and raises an uncaught `TypeError` — the budget bug in a second module, found the same day that one was ruled. Seven mutations; **`TrimSpace` for `str.strip()` survived** until a row was cut for it, since U+001C–U+001F are whitespace to Python and not to Go's `unicode.IsSpace`. Proved on the real wire by a live case of a **blank grey square**, which is the honest test: the correct answer is two empty strings, and a model that fills them in anyway is the one failure ADR 34's instructions exist to prevent |
  | `api/argueruns.py` — the slot argument swept across a selection | **go** | `go/internal/api/argue.go`, 2026-08-23 — the mode already crossed with its plain route; this is the multiplication, and a **job** because one call per card makes a few dozen slots minutes. One job for the whole sweep and sequential inside it, since N jobs would occupy the two-wide NET lane and starve every sibling surface. A failed card is recorded and the sweep carries on — partial results are the point of paying for one — with the single exception of the credential vanishing mid-run, where every remaining card would fail the same way and the rest are marked unattempted. **`errors` is a `wire.OrderedMap`**: it is a dict built in *sweep* order, so a Go map would alphabetise a list whose order is the order things went wrong in — the Notes-tab regression wearing its fourth hat. **`PyCasefold` and `PyStrip` are newly exported for it**, and both because this is the one comparison in the family where the two sides are *different strings*: a name somebody typed against the deck's own spelling. CLAUDE.md exempts the neighbours from `casefold` precisely because they fold a pool name against the pool's own; this one is not exempt. Six mutations, all dead — including `TrimSpace` for `str.strip()`, which needed a name wrapped in U+001C and U+001F cut for it. **One gap is recorded rather than arranged**: no fixture card holds a character where `casefold` and `lower` disagree, so a `ToLower` mutation survives; manufacturing a card for it would be arranging the evidence |
  | `/api/health`, `GET /api/sets/upcoming`, `POST .../wheel` — the app's last three | **go** | `go/internal/api/health.go`, `sets.go`, `wheel.go` + `go/internal/wheel` and `pool.Stale`, 2026-08-23 (Phase 8's first route batch). The wheel is `pyrand`'s second served caller — a seed replayed across the cutover deals the same fate, face and card, held to Python by the generated `wheel/testdata/spins.json` compared as marshalled bytes; the sets filter is held by `api/testdata/sets.json`, rendered through the real `service.upcoming_sets` with the clock frozen and the network stubbed. Two behaviours worth the row: `int(seed)` over a value Python's grammar refuses is an **uncaught 500** — Starlette's plain-text three words, measured before written, reproduced by `pythonUncaught` — and `/api/health` answered by the door alone ends the per-deploy boot window where the platform's probe met a proxied route with no uvicorn behind it yet (40 ERROR lines on the v-bump this landed beside) |
  | everything else under `/api` — `cli.py` | python | proxied to uvicorn on loopback; the two generic job routes have their own row above, since the reason they stay is not the reason these do. **The whole Claude family has crossed** as of 2026-08-23: all twelve routes, all seven modes — and with Phase 8's first batch the only Python literals left are the admin stats six and the account delete, the rows above |
