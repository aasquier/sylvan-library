...
paragraph was written before the Claude family had a phase of its own. The shim itself re-ships as a static Go binary on the
...
- **The port board is the table below**: per module — `python | porting |
  ...
  | `POST /api/sim/forge` — Tier 3 | python | **not part of the sim flip, despite the prefix.** It needs `sim/tier3`, which is Phase 7's engine and has not crossed. It is also, since #263 closed the Claude family, **the only thing keeping the hybrid poll handler's proxy branch live at all** -- every other job-submitting family is the door's now, so `TestAJobIdTheDoorDoesNotHoldIsProxied` rests on this one route until Phase 7 crosses it |
