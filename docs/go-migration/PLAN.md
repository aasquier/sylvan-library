...
- **The port board is the table below**: per module — `python | porting |
  ...
  | everything else under `/api` — `api/` (two of the eight job-submitting families: scan and the slot-argument sweep, plus `/api/claude`, `/api/health`, the upcoming sets), `decks/` (the wheel), the rest of `claude/` (scan), `cli.py` | python | proxied to uvicorn on loopback; the two generic job routes have their own row above, since the reason they stay is not the reason these do |
