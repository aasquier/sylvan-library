...
- **The port board is the table below**: per module — `python | porting |
  ...
  | everything else under `/api` — `api/` (`/api/health`, the upcoming sets), `decks/` (the wheel), `cli.py` | python | proxied to uvicorn on loopback; the two generic job routes have their own row above, since the reason they stay is not the reason these do. **The whole Claude family has crossed** as of 2026-08-23: all twelve routes, all seven modes |
