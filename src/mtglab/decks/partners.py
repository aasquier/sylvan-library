...
The gate assumed one commander. That produced two wrong answers on perfectly
legal decks:

* A Background is a `Legendary Enchantment — Background`. It is not a creature
  and its text never says it can be your commander, so `not-a-commander`
  rejected Jaheira + Raised by Giants -- a legal Baldur's Gate pairing.
* Ten `Partner with` cards from Battlebond (Lore Weaver, Ley Weaver, Chakram
  Slinger and friends) are not even Legendary. CR 702.124f grants commander
  eligibility through the ability itself rather than printing it on the card,
  so reading the type line alone rejects them too.
...
def can_be_commander(rec: Any, *, paired: bool = False) -> bool:
    ...
    `paired=True` when it is one of two commanders, which is what makes a
    Background or a non-legendary `Partner with` card legal.
    """
    ...
    if "can be your commander" in (rec.oracle_text or "").lower():
        return True
    if paired:
        # CR 702.124f: partner with grants commander eligibility, which is why
        # the non-legendary Battlebond partners are legal.
        if is_background(rec):
            return True
        p = pairing(rec)
        if p is not None and p.kind == PARTNER_WITH:
            return True
    return False
