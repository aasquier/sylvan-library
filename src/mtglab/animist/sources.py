...
def confirm(source: Source, *, transport: Transport | None = None) -> Confirmed:
    ...
    if source.provider == "procedural":
        return _confirm_procedural(source)
...
def _confirm_procedural(source: Source) -> Confirmed:
    """The gate's answer for pixels with no upstream (ADR 31).

    There is no provider to ask and nothing to download: the source is its
    own declaration — a seed — and the licence is necessarily ours. The
    confirmation object still exists so provenance renders the same shape
    for every source, dated like any other pass; what it records is that
    the gate had nothing to gate, not that it was skipped.
    """
    from datetime import date

    if source.seed is None:
        raise SourceError(f"source `{source.id}`: procedural without a seed "
                          "-- the loader should have refused this recipe")
    return Confirmed(
        confirmation=LicenceConfirmation(
            source_id=source.id,
            licence="ours-generated",
            checked_on=date.today().isoformat(),
            api_url="(none -- generated by this pipeline, no upstream)",
        ),
        files=(),
    )
